...
(ns cljs-browser-repl.mathbox
  (:require ;[cljs.core.async :as async]
   [cljsjs.mathbox]
   #_[mathbox]
   #_[cljsjs.mathbox.common.mathbox]
   )
  #_(:require-macros [cljs.core.async.macros :refer [go]]))

#_(def ThreeBox (js-obj :preload (fn [element options] )))

#_(aset js/window "mathBox" (fn [element options] ))

#_(def MathBox (js-obj "start" (fn [])
                     "platonic" (fn [options])
                     :curve (fn [options] )
                     :bezier (fn [options] )
                     :bezierSurface (fn [options] )
                     :grid (fn [options] )
                     :axis (fn [options] )
                     :surface (fn [options] )
                     :viewport (fn [options] )
                     :camera (fn [options] )
                     :animate (fn [object attributes options] )
                     :clone (fn [selector options animate] )
                     :vector (fn [options] )
                     :remove (fn [])
                     :get (fn [selector] )
                     :set (fn [selector options] )))
...
(defn create-mathbox
  ...
  (let [defaults {:cameraControls true
        ...
        mb
        (if containing-element
              (.start (aget js/window "mathBox") containing-element
                (clj->js (into defaults opts-map)))
              (.start (aget js/window "mathBox") (clj->js (into defaults opts-map))))
        #_(if containing-element
